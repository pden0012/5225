...
def analyze_file(item):
    ...
    # 使用 process_start_time 来记录处理开始时间
    process_start_time = datetime.datetime.now()  # 改名以避免冲突
    offset = 0
    ...
    # 转换结果为列表格式
    bird_detections = []
    for time_range, detections in results.items():
        segment_start, segment_end = map(float, time_range.split('-'))  # 使用不同的变量名
        for label, confidence in detections:
            bird_detections.append({
                'species': label,
                'confidence': float(confidence),
                'start_time': segment_start,  # 使用新的变量名
                'end_time': segment_end      # 使用新的变量名
            })

    # 使用 process_start_time 计算处理时间
    delta_time = (datetime.datetime.now() - process_start_time).total_seconds()
